...
One CodeMirror `ViewPlugin` owns the independent virtual-number and stored-number-concealment
features for each `EditorView`. Either or both can be enabled. It:
...
- places a replacement virtual widget after a concealed prefix when both features are enabled;
- removes both decorations when any selection touches a concealed heading line or during IME composition; and
...
Virtual numbers are prepended using DOM node APIs. Concealment validates the exact leading text before wrapping it. Combined display only prepends the virtual replacement after concealment succeeds. No heading content is passed to `innerHTML`.
...
6. replace each exact previewed source through `Vault.process`; and
7. roll back only files that still contain the exact plugin-written result after an error.

Recovery accepts files that still match either the exact before-state or verified after-state digest, so it can repair a process interrupted halfway through a batch without duplicating both full file versions. Any independently edited file cancels recovery before writes begin; an edit that races with write or rollback is preserved and leaves the recovery snapshot available. If an interrupted promotion leaves both recovery files, the newer pending file is validated first and the stable file remains the fallback.
...
The canonical check reports coverage across every runtime source file, with stricter thresholds for the pure core, application plans, recovery adapter, and Reading View. The release checker verifies versions, manifest identity, static asset freshness, bundle size, runtime externals, absence of production source maps, and a dated accepted Android platform record whenever mobile loading is enabled. Release-specific runtime acceptance remains a separate manual gate.
